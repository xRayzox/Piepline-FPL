...
<style>
    body {
        color: #333;  /* Dark gray text color for light theme */
...
# ... (Rest of the data preprocessing code is the same as before)
...
next_unfinished_gameweek = next(
    (
        ...
        if df_fixtures[
            (df_fixtures["event"] == gw) & (df_fixtures["finished"] == False)
        ].shape[0]
        > 0
    ),
    min_gameweek,
...
if selected_display == "Premier League Fixtures":
    ...
    current_gameweek_fixtures = df_fixtures[
        df_fixtures["event"] == selected_gameweek
    ]
    ...
    for date, matches in grouped_fixtures:
        ...
        for _, match in matches.iterrows():
            col1, col2, col3 = st.columns(
                [2, 1, 2]
            )  # Adjust columns for team names and score
            ...
            with col2:
                if match["finished"]:
                    ...
                    st.markdown(
                        f"<p style='text-align: center;'>vs</p>",
                        unsafe_allow_html=True,
                    )
            with col3:
            ...
            if not match["finished"]:
                st.caption(
                    f"Kickoff: {match['local_hour']}"
                )  # Display kickoff time as a caption
...
# --- FDR Matrix Display ---
    ...
    filtered_fdr_matrix = filtered_fdr_matrix[
        [
            ...
            if f"GW{gw}" in filtered_fdr_matrix.columns
        ]
    ]
