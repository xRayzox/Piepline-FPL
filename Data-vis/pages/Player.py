...
plt.scatter(df_merged['now_cost'], df_merged['total_points'], color='blue', alpha=0.5)
...
for i, player in enumerate(df_merged['web_name']):
    plt.annotate(player, (df_merged['now_cost'][i], df_merged['total_points'][i]), textcoords="offset points", xytext=(0,10), ha='center')
...
fig = px.scatter(df_merged, x='now_cost', y='total_points', text='web_name', title='Player Performance: Cost vs Total Points')
fig.update_traces(textposition='top center')  # Position text labels
fig.update_layout(xaxis_title='now_cost', yaxis_title='Total Points')
